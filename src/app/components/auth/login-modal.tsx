...
import { ArrowRight, Bot, BrainCircuit, Building2, ShieldCheck, Stethoscope, UserRound, X } from "lucide-react";
...
export function LoginModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/35 p-4 backdrop-blur-md">
      <div className="relative grid w-full max-w-[940px] overflow-hidden rounded-[34px] border border-white/70 bg-white/90 shadow-[0_28px_90px_rgba(44,71,146,0.22)] lg:grid-cols-[0.92fr_1.08fr]">
        <button
          type="button"
          onClick={onClose}
          className="absolute right-5 top-5 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/80 text-slate-500 shadow-sm hover:bg-slate-50"
          aria-label="Đóng"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <section className="relative hidden overflow-hidden bg-gradient-to-br from-[#eef5ff] via-[#e8efff] to-[#dfe4ff] p-8 lg:block">
          ...
          <div className="relative">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-extrabold text-blue-600">VitaCare AI</h2>
                <p className="text-sm font-semibold text-slate-500">Hệ thống Y tế Thông minh</p>
              </div>
            </div>
            ...
            <div className="mt-12 rounded-[26px] border border-white/80 bg-white/65 p-5 shadow-[0_18px_45px_rgba(72,98,170,0.14)] backdrop-blur">
              <div className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-r ${current.accent} text-white shadow-lg`}>
                <CurrentIcon className="h-7 w-7" />
              </div>
              <p className="mt-5 text-sm font-bold uppercase tracking-[0.24em] text-blue-500">Đăng nhập theo vai trò</p>
              <h3 className="mt-2 text-3xl font-extrabold leading-tight text-slate-800">{current.title}</h3>
              <p className="mt-4 text-sm leading-7 text-slate-500">
              Truy cập đúng không gian làm việc cho từng vai trò, đồng bộ với trợ lý AI ở trang đầu.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="p-6 sm:p-8">
          <div className="pr-12">
            ...
            <h2 className="mt-4 text-3xl font-extrabold text-slate-800">Đăng nhập / Đăng ký</h2>
            <p className="mt-2 text-sm leading-6 text-slate-500">Chọn vai trò, kiểm tra tài khoản mẫu và tiếp tục vào hệ thống.</p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            {(Object.keys(roles) as LoginRole[]).map((item) => {
              ...
              return (
                ...
                >
                  <div className={`flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-r ${roles[item].accent} text-white`}>
                    <RoleIcon className="h-5 w-5" />
                  </div>
                  <div className="mt-3 text-sm font-extrabold text-slate-800">{roles[item].label}</div>
                  <div className="mt-1 text-xs font-medium leading-5 text-slate-500">{roles[item].title}</div>
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-6 rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 flex items-center justify-between gap-3 text-sm">
              <span className="font-semibold text-slate-500">Tài khoản mẫu: {current.name}</span>
